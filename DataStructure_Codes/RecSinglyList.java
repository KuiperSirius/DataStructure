...
public class RecSinglyList<T> extends Object               //单链表类，T为数据元素类型，第4章
{
    ...
    private String toString(Node<T> p)           //返回从p结点开始的子单链表描述字符串，递归方法
    {
         ...
         if (p.next!=null) 
             str+=", "; 
         return str+this.toString(p.next);       //递归调用，递归通式：当前结点元素串连接从p的后继结点开始的子单链表串
    }
    ...
    public int size(Node<T> p)                   //返回从p结点开始的单链表长度，递归方法
    {
        if (p==null)                             //递归结束条件：空单链表的长度为0
            return 0;
        return 1+size(p.next);                   //递归调用，递归通式：当前结点计数为1，加上从p的后继结点开始的子单链表长度
    }
    ...
    {
        Node<T> q=null;
        if (p!=null)                             //存在p结点，递归结束条件 
        {
            q = new Node<T>(p.data, null);       //创建与p结点同值的结点
            q.next = copy(p.next);               //复制从p结点开始的子单链表，链入q结点之后。递归调用 
        }
        return q;
    }
}
